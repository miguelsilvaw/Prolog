:- dynamic(fact/1), [forward,proof,basedadosv2].
:-initialization(menu).
...
resultadowrite(P) :- dados(A,B,C,D,E,F,G,H,I,J,K,L,M,N,O), demo(P, Proof), nl,
	write('As suas opções de escolha foram:'), nl,
	write(Proof), nl,
	write(''), nl,
	write('Consoante as suas escolhas, aconselhamos:'), nl,
	write(''), nl,
	write('ID: '), write(A), nl,
	write('Festival: '), write(E), nl,
	write('Passe Geral: '), write(F), nl,
	write('Bilhete Individual: '), write(G), nl,
	write('Preço Médio: '), write(K), nl,
	write('Música: '), write(D), nl,
	write('Estação: '), write(H), nl,
	write('Região: '), write(M), nl,
	write('Campismo: '), write(J), nl,
	write('Ambiente: '), write(L), nl,
	write('Diversão: '), write(N), nl,
	write('País: '), write(B), nl,
	write('Cidade : '), write(C), nl,
	write('Duração (em dias): '), write(I), nl,
	write('Número de palcos: '), write(O), nl,
	write('').
